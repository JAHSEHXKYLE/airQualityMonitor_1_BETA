...
#ifndef _GxEPD2_2IC_EPD_H_
#define _GxEPD2_2IC_EPD_H_

#include <Arduino.h>
#include <SPI.h>

#include <GxEPD2_2IC.h>

#pragma GCC diagnostic ignored "-Wunused-parameter"
//#pragma GCC diagnostic ignored "-Wsign-compare"

class GxEPD2_2IC_EPD
{
  public:
    // attributes
    ...
    // constructor
    GxEPD2_2IC_EPD(int16_t cs, int16_t cs1, int16_t dc, int16_t rst, int16_t busy, int16_t busy_level, uint32_t busy_timeout,
               uint16_t w, uint16_t h, GxEPD2::Panel p, bool c, bool pu, bool fpu);
    virtual void init(uint32_t serial_diag_bitrate = 0); // serial_diag_bitrate = 0 : disabled
    virtual void init(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration = 10, bool pulldown_rst_mode = false);
    virtual void csselect(uint8_t n, uint8_t m = 0);
    ...
    virtual void setPaged() {}; // for GxEPD2_2IC_154c paged workaround
  ...
  protected:
    ...
    void _writeCommand(uint8_t c, uint8_t m = 0);
    void _writeData(uint8_t d, uint8_t m = 0);
    void _writeData(const uint8_t* data, uint16_t n, uint8_t m = 0);
    void _writeDataPGM(const uint8_t* data, uint16_t n, int16_t fill_with_zeroes = 0, uint8_t m = 0);
    void _writeDataPGM_sCS(const uint8_t* data, uint16_t n, int16_t fill_with_zeroes = 0, uint8_t m = 0);
    void _writeCommandData(const uint8_t* pCommandData, uint8_t datalen, uint8_t m = 0);
    void _writeCommandDataPGM(const uint8_t* pCommandData, uint8_t datalen, uint8_t m = 0);
    void _startTransfer(uint8_t m = 0);
    void _transfer(uint8_t value);
    void _endTransfer(uint8_t m = 0);
  protected:
    int16_t _cs, _cs1, _dc, _rst, _busy, _busy_level;
