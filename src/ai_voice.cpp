...
void InitIot() {
  ...
  std::vector<ai_vox::iot::Property> indoor_air_quality_properties({
      {
          "pm25",  // property name
          "室内PM2.5浓度",  // property description
          ai_vox::iot::ValueType::kNumber  // property type
      },
      {
          "pm10",  // property name
          "室内PM10浓度",  // property description
          ai_vox::iot::ValueType::kNumber  // property type
      },
  ...
  indoor_air_quality_iot_entity = std::make_shared<ai_vox::iot::Entity>("IndoorAirQuality", 
                                                                 "室内空气质量检测器", 
                                                                 std::move(indoor_air_quality_properties),
                                                                 std::move(indoor_air_quality_functions)
 );
  indoor_air_quality_iot_entity->UpdateState("pm25", 100);
  indoor_air_quality_iot_entity->UpdateState("pm10", 100);
        ...
        if (auto activation_event = std::get_if<ai_vox::Observer::ActivationEvent>(&event)) {
            ...
            if (iot_message_event->name == "IndoorAirQuality") {
                if (iot_message_event->function == "GetIndoorAirQuality") {
                    ...
                    Serial.println("GetIndoorAirQuality");
                    indoor_air_quality_iot_entity->UpdateState("pm25", 10);
                    indoor_air_quality_iot_entity->UpdateState("pm10", 52);
                }
            }
